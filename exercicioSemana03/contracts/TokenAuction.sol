...
contract TokenAuction {
  ...
  function verifyFinished(OneAuction storage a) private {
    ...
    //Função que altera o valor da taxa do contrato
    function changeFee( uint fee) public{
      //verifica se quem chamou a função foi o dono do contrato, caso seja o valor da taxa é alterada
      require(msg.sender == owner, "You must own the contract to change the fee!");
      contractFee = fee;
    }

    //Função auxiliar para verificar o valor da taxa, para testar se ela foi alterada
    function verifyFee() public view returns (uint){
      return contractFee;
    }
